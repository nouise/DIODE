...
if __name__ == '__main__':
    ...
    project_root = opt.out if opt.out else opt.project
    exp_name = opt.name or 'exp'
    base_dir = os.path.join(project_root, exp_name)
    os.makedirs(base_dir, exist_ok=True)
    # weights and results paths (override module-level defaults)
    wdir = os.path.join(base_dir, 'weights') + os.sep
    ...
    # Prepare project/output directories
    # If --out is provided it overrides --project; outputs are organized as: <project>/<name>/
    ...
    if not opt.evolve:  # Train normally
        print('Start Tensorboard with "tensorboard --logdir=%s", view at http://localhost:6006/' % os.path.join(base_dir, 'runs'))
        tb_writer = SummaryWriter(log_dir=os.path.join(base_dir, 'runs'))
