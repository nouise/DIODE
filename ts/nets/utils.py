...
def output_dataset(train_loader,mean,std,out_dir='imgs/mnist'):
    mean = torch.Tensor(mean).view(1,-1,1,1)
    std = torch.Tensor(std).view(1,-1,1,1)
    import os
    os.makedirs(out_dir,exist_ok=True)
    img_dict = {idx:0 for idx in range(10)}
    for i, contents in enumerate(train_loader):
        label = contents[1]
        data = torch.clip(contents[0] * std + mean,0,1)*255
        data = data.type(torch.uint8)
        for j in range(data.shape[0]):
            plab = int(label[j].item())
            pdata = data[j,0].numpy()
            iidx = img_dict[plab]
            img_dict[plab] += 1
            out_name = f'{plab}_{iidx}.png'
            img = Image.fromarray(pdata)
...
def test(test_loader, network, criterion, epoch, device, print_freq=10):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
...
class Timer:
    ...
    def print(self):
        current = time.time()
        out_str = f'time eslipse:{(current-self.last)}'
        self.last = current
        return out_str
...
class AverageReporter:
    ...
    def __init__(self,keys):
        self.mdict = {pk:[] for pk in keys}
        
    def record(self,val_dict):
        for pk in val_dict.keys():
            assert pk in self.mdict.keys(), ('the features to record should be intialized in contructuor')
            self.mdict[pk].append(val_dict[pk])
    
    def report(self):
        res = '\t'.join([f'{pk}_{(sum(self.mdict[pk])/len(self.mdict[pk])):.5f}' for pk in self.mdict.keys()])
        for pk in self.mdict.keys(): self.mdict[pk].clear()
        return res
