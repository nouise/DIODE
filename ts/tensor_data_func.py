...
class CoolChicEncoder(nn.Module):
    ...
    def forward(self,
        ...
        decoder_side_latent = [ts.view(sz) for ts,sz in zip(torch.split(flat_decoder_side_latent,self.size_per_latent_flat),self.size_per_latent)]
...
class DataOptim:
    ...
    def init_solver_dg(self,dg:DGrids)->None:
    ...
    def init_solver_dg_list(self,dg:list[DGrids])->None:
        op_module = ["grids"] if "all" in self.phase.optimized_module or "latent" in self.phase.optimized_module else []
        for dgo in dg:
...
class TensorData(nn.Module):
    ...
    def __init__(self, lmbda=0.001, n_itr=20000, image_size=(512,512), batch_size=1, device='cpu') -> None:
        super().__init__()
        self.param = CoolChicEncoderParameter(device=device,batch_size=batch_size)
    ...
    @torch.no_grad()
    def mimic_forward(self,grids:DGrids, dp:DParams, quantizer_noise_type,quantizer_type, qa, qb):
        output,noise = self.gen.forward_data(grids.pool['grids'], dp.pool['up'], dp.pool['sp'],
            quantizer_noise_type, quantizer_type, qa,qb)      
        return output,noise
    
    @torch.no_grad()
    def forward_for_test(self,grids:DGrids, dp:DParams):
        self.set_to_eval()
        output,rate = self.gen.forward(grids.pool['grids'],dp.pool['ap'], dp.pool['up'], dp.pool['sp'],
    ...
    def forward(self,grids:DGrids, dp:DParams,quantizer_noise_type,quantizer_type, qa, qb):
        output = self.gen.forward(grids.pool['grids'],dp.pool['ap'], dp.pool['up'], dp.pool['sp'],
            quantizer_noise_type, quantizer_type, qa, qb)      
        return output
    
    def forward_per_sample(self,grids:DGrids, dp:DParams,noise,quantizer_noise_type,quantizer_type, qa, qb):
        output = self.gen.forward_per_sample(grids.pool['grids'],dp.pool['ap'], dp.pool['up'], dp.pool['sp'], noise, quantizer_noise_type,quantizer_type, qa, qb)      
        return output
    
    @torch.no_grad()
    def test(self, dg:DGrids, dp:DParams, ref:torch.Tensor, loss_func):
    ...
    def set_training_phase(self, dg:DGrids, dp:DParams, phase_idx, warmup=False):
        dop = DataOptim(self.manager,phase_idx,warmup)
        dop.init_solver_dg(dg)
        dop.init_solver_dp(dp)
        return dop
            
    def pretrain(self, dg:DGrids, dp:DParams, loss_func, ref):
    ...
    def produce_parameters(self):
        return self.gen.initialize_latent_grids(), self.gen.initialize_parameters()
