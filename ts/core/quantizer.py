...
def quantize(
    ...
    noise_out:Optional[Tensor|None] = None,
    ...
    match quantizer_type:
        case "none":
            if output_noise:  return x + noise,noise
            return x + noise
        case "softround_alone":
            if output_noise: return softround(x, soft_round_temperature), None
            return softround(x, soft_round_temperature)
        case "softround":
            if output_noise: return softround(softround(x, soft_round_temperature) + noise, soft_round_temperature,),noise
            return softround(
                softround(x, soft_round_temperature) + noise,
                soft_round_temperature,
            )
        case "ste":
            # Forward: hard round. Backward: softround derivative.
            y_soft = softround(x, soft_round_temperature)
            ...
            return y
        case "hardround":
            if output_noise: return torch.round(x), None
...
def round_dgm(x: torch.Tensor, beta: float = 1.0) -> torch.Tensor:
    """DGM-style straight-through rounding.

    Forward: round(x)
    Backward: gradients come from a smooth proxy function on the fractional part.
    """
    x_round = torch.round(x)

    # Fractional part in [0, 1) with integer part detached.
    x_floor_detached = torch.floor(x).detach()
    frac = x - x_floor_detached

    # Smooth proxy for the fractional part.
    # Maps frac=0 -> ~0, frac=0.5 -> 0.5, frac=1 -> ~1 (after normalization).
    beta_t = torch.as_tensor(beta, dtype=x.dtype, device=x.device)
    denom = torch.tanh(0.5 * beta_t)
    denom = denom + (denom == 0).to(dtype=denom.dtype) * 1e-12
    frac_smooth = (torch.tanh(beta_t * (frac - 0.5)) / denom) / 2 + 0.5

    proxy = frac_smooth + x_floor_detached

    # Same pattern as round_ste: forward == x_round, backward == proxy
    return proxy + (x_round - proxy).detach()

def ste_round_to_bitdepth(x: Tensor, bitdepth: int) -> Tensor:
    """STE rounding of a tensor to a given bitdepth in [0,1] space.

    Forward: round(x * (2^bitdepth - 1)) / (2^bitdepth - 1)
    Backward: identity
    """
    max_dynamic = float(2**bitdepth - 1)
    y = torch.round(x * max_dynamic) / max_dynamic
    return x + (y - x).detach()
