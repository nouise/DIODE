...
class TensorPool:
    ...
    def __init__(self,nclass,slice_size,sample_per_class:list[int], cuda_devices_idxs:list[int], nthread = 4,
        ...
        self.worker_pool = {tidx: TensorData(image_size=img_size,batch_size=self.slice_size,  
                                             device='cuda:{}'.format(cuda_devices_idxs[tidx%len(cuda_devices_idxs)])) for tidx in range(sum(self.slice_per_class))}
    ...
    def set_for_cls(self):
        self.cls = True
        
    def set_for_mse(self):
        self.cls = False 
    ...
    def __next__(self):
        if self.bidx < self.nbatch:
            ...
            if not self.cls:
                ...
                cls_label = torch.Tensor([pa for pa,_ in idxs]).type(torch.long)
                return data,rate,cls_label
        else:
            raise StopIteration
    ...
    def load_from_files(self,root,prex):
        for cidx in range(self.nclass):
            for sidx in range(self.slice_per_class[cidx]):
                mdict = torch.load(f'{root}/{prex}_{cidx}_{sidx}.pt',map_location='cpu',weights_only=False)
                self.slice_pool[f'{cidx}_{sidx}']['param'].set_params(mdict['param'],'cpu')
                
                 
    def save_slice_pool(self,pool_path_dir='./mnist'):
        self.load_best()
        for pk in self.slice_pool.keys():
            torch.save({'param':self.slice_pool[pk]['param'].get_params()},f'{pool_path_dir}/sp_{pk}.pt')
    ...
    def shuffle_idxs(self):
        self.bidx = 0
        self.epoch += 1
        idxs_pool = [(cls,sidx) for cls in range(self.nclass) for sidx in range(self.sample_per_class[cls])]
        #if not self.testing: 
        random.shuffle(idxs_pool)
        self.schedule = []
        for idx in range(self.nbatch):
            ...
            for idy in range(self.batch_size):
                ...
                cls,sidx =  idxs_pool[pid]
                bidx,cidx = sidx // self.slice_size, sidx % self.slice_size
                pk = f'{cls}_{bidx}'
